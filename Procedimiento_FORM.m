...
num_archivo = input('Ingrese un número para identificar los archivos de salida: '); % para diferenciar los documentos de salida en caso de multiples analisis

% Generar los nombres de los archivos
nombre_resultados = sprintf('Resultados de FORM caso 1_%d.xlsx', num_archivo); % decidir como se llamarán los docs de salida
...
% Leer archivos xls
bias_cov = xlsread(filePath, 'bias y CoV para matlab', 'A2:I3'); % extraer factor bias y coeficientes de variación
bias=bias_cov(1,:);
cov=bias_cov(2,:);

% Leer datos de diseño de las vigas 
valores_diseno = xlsread(filePath, 'Datos vigas para matlab', 'A3:I322'); % indicar rango de celdas con valores de diseño
...
% Se debe generar la ecuación de estado límite de manera simbólica
% indicar las variables EN ORDEN
% escribir la ecuación de estado límite en la variable M
...
limit_func = @(in1) M_func(in1); % sirve para evaluar la ecuación de estado límite
...
for i = 1:num_vigas
    ...
    resultadosFORM(i, :) = [Pf*100, Beta]; % las probabilidades ya serán dadas en porcentaje
